...
```{r, eval=FALSE}
efficiency_ratio <- function(hp, wt) {
  ratio <- hp / wt
  return(ratio)
}

hp_to_wt_ratios <- numeric(nrow(mtcars))

for (i in 1:nrow(mtcars)) {
  hp_to_wt_ratios[i] <- efficiency_ratio(mtcars$hp[i], mtcars$wt[i])
}

mtcars$hp_to_wt_ratio <- hp_to_wt_ratios
